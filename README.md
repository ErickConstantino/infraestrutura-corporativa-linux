...
1. No terminal do Debian, faça login como administrador (`root`):
   ```bash
   su -
Abra o arquivo de configuração de rede.
(Se você for leigo, use o editor nano, se quiser aprender comandos profissionais, use vim):
...
(Para deixar definitivo, abra o arquivo /etc/sysctl.conf, procure a linha #net.ipv4.ip_forward=1 e apague o símbolo # para desfragmentá-la).
...
Diga ao DHCP em qual placa de rede ele deve escutar. Abra o arquivo de configuração de interfaces:
...
Modifique o conteúdo para que ele aceite requisições da sua rede e encaminhe o que não souber para os DNS públicos da Cloudflare (1.1.1.1) e Google (8.8.8.8):
...
[SUPER TRUQUE DE DEBUG]: Antes de reiniciar, verifique se você não esqueceu nenhum ponto e vírgula ; correndo o comando de validação:

Bash
named-checkconf
(Se o comando não retornar absolutamente nada, parabéns! Sua sintaxe está perfeita).

Ative o serviço usando o nome real do processo de segundo plano do sistema (named):
...
DOS
ping google.com
Se os pacotes responderem, a sua infraestrutura de redes corporativa está completa, funcional e pronta para produção! 🚀
